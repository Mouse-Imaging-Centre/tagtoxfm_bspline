...
#ifndef lint
static char rcsid[] = "$Header: /software/source/libraries/EBTKS/src/TBSpline.cc,v 1.8 2004/04/06 18:52:33 bert Exp $";
#endif
                                         ...
                                         // false -> cannot call add_data 
{
  ...
  if(domain.cols() != 2 || distance <= 0 )
    return;  // fatal error

  _nDimensions = domain.rows();
  ...
  _zero.resize(_nDimensions);
  _zero = _knots(3, blitz::Range::all());
  ...
  if(allocate_flag) {
    ...
    _AtF.resize(_nProduct);
  }
  _coef.resize(_nProduct);
  _nCoef = _nProduct;
  _terms.resize(4,_nDimensions);
  _blockIndex.resize(_nDimensions);
  _values.resize(_four);
  _locations.resize(_four);
...
TBSpline::addDataPoint(const float *point, double value)
{
  ...
  AtAel = (double *) _AtA.data();
  AtFel = (double *) _AtF.data();
...
TBSpline::fit(void)
{
  ...
  _coef = solveSymmetricSystem(A, _AtF,&info);
...
DblArray
TBSpline::solveSymmetricSystem(DblMat &A, DblArray b, int *info)
{
  ...
  dsysv_(&uplo, &n, &nrhs, (_doublereal *) A.data(), &lda, ipiv, 
	 (_doublereal *) b.data(), &ldb,
...
TIndex::init(const IntArray &n)
{
  ...
  _n = n;
  _flat = 0;
  _index.resize(_nDimensions);
  _index = 0;

  // calculate step sizes for indexing an array as a tensor
  _step.resize(_nDimensions);  
  for(i = 0; i < _nDimensions; i++)
    {
      _step(i) = 1;
...
{
  _smallN.resize(smallN.shape());
  _smallN = smallN;
  _smallIndex.resize(_nDimensions);
  _smallIndex = 0;
  _smallStep.resize(_nDimensions);
  unsigned int i, j;
...
void test_TBSpline()
{
  DblMat domain(3,2);
  domain =  0, 5, 0, 4, 0, 7;
  TBSpline A(domain, 0.5, 0.001, true);
  
  float point[] = {0, 2, 3};
  A.addDataPoint(point, 3.0);
  A.fit();
  A(0.5, 2.0, 3.0);
                             ...
                             const int count[VDIM])
{
  ...
  _sizes.resize(VDIM);
  _sizes = IntArray((int *) count, blitz::shape(VDIM), blitz::neverDeleteData); 
...
TBSplineVolume::operator () (int x, int y, int z) const
{
  ...
  double value, xv, xyv;
  int xo, xyo, i, j, k;
  const double *coef = _coef.data();  // direct access to array
